"""
Process running animations from subdirectories
...
DIRECTIONS_MAP = {
    'run_n': 0, 'north': 0,
    'run_ne': 1, 'northeast': 1,
    'run_e': 2, 'east': 2,
    'run_se': 3, 'southeast': 3,
    'run_s': 4, 'south': 4,
    'run_sw': 5, 'southwest': 5,
    'run_w': 6, 'west': 6,
    'run_nw': 7, 'northwest': 7,
}
...
def process_running_from_subdirectories():
    print("=" * 70)
    print("Processing Running Animations from Subdirectories")
    print("=" * 70)
    print(f"\nSource: {SOURCE_DIR}")
    ...
    run_dirs = [d for d in SOURCE_DIR.iterdir() if d.is_dir() and d.name.startswith('run_')]
    if not run_dirs:
        print("\n[INFO] No running animation subdirectories found.")
        print("Expected directories like: run_n, run_ne, run_e, etc.")
        return

    print(f"\nFound {len(run_dirs)} running animation directories")

    for run_dir in sorted(run_dirs):
        dir_key = run_dir.name
        direction_id = DIRECTIONS_MAP.get(dir_key)
        if direction_id is None:
            print(f"  [SKIP] Unknown running direction directory: {dir_key}")
            continue
        ...
        bmp_files = sorted(run_dir.glob("*.bmp"))
        if not bmp_files:
            # Also try PNG files
            bmp_files = sorted(run_dir.glob("*.png"))
        
        if not bmp_files:
            print(f"  [SKIP] No image files found in {run_dir}")
            continue
        ...
        # Running animations typically have multiple frames (similar to walking)
        ...
        output_file = OUTPUT_DIR / f"male_run_{dir_name}_sheet.png"
        sheet.save(output_file, "PNG")
        print(f"\n  [SUCCESS] Saved: {output_file.name}")

        # Also save as standard run if it's east (default direction)
        if direction_id == 2:  # East is often the default/standard run sheet
            standard_file = OUTPUT_DIR / "male_run_sheet.png"
    ...
    print("=" * 70)
    print(f"Successfully processed: {len(run_dirs)}/{len([k for k in DIRECTIONS_MAP.keys() if k.startswith('run_')])} directions")
    print("=" * 70)

if __name__ == "__main__":
    process_running_from_subdirectories()
